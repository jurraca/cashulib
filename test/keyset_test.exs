defmodule KeysetTest do
  ...
  describe "NUT-01 test vectors" do
    ...
    test "handles a big integer amount in the keyset denomination" do
      valid_keyset_id = "00456a94ab4e1c46"
      valid_keyset = %{
        "1" => "03a40f20667ed53513075dc51e715ff2046cad64eb68960632269ba7f0210e38bc",
        "2" => "03fd4ce5a16b65576145949e6f99f445f8249fee17c606b688b504a849cdc452de",
        "4" => "02648eccfa4c026960966276fa5a4cae46ce0fd432211a4f449bf84f13aa5f8303",
      ...
      valid_keyset_big_id = "000f01df73ea149a"
      valid_keyset_big = %{
      ...
      assert Keyset.valid_keys?(valid_keyset)
      assert Keyset.derive_keyset_id(valid_keyset) == valid_keyset_id
      assert Keyset.valid_keys?(valid_keyset_big)
      assert Keyset.derive_keyset_id(valid_keyset_big) == valid_keyset_big_id
    end
  end
end
